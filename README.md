...
没有push master。都不会影响github上的master。v0.1 和v0.3 的readme.md与本文不符就是证明。
    ...
    所以 
      1.请在需要tag时，commit后再tag出一个时间点（一个分支），push完tag后请立刻回到原来所在分支（branch）。
      2.或者 tag + commit号来指定项目时间点来打标签。
...
##2020-2-1
###channel
向管道输入 阻塞直至数据被输入至缓冲区（当缓冲区为0或者缓冲区不足都阻塞）
>c<-'A'
...
>   var a inter=2; \
>   var b LessAddoneer = &a;    // yes
>>    var b LessAddoneer = a;// error  \
>
